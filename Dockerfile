...
##############################################################################
# Configure git so private uug-ai modules resolve during the build.
...
RUN cd /go/src/github.com/uug-ai/${project} && \
    go mod download && \
    go build -o main -tags timetzdata,netgo --ldflags '-s -w -extldflags "-static -latomic"' . && \
...
FROM alpine:latest
LABEL org.opencontainers.image.source https://github.com/uug-ai/hub-loitering
LABEL AUTHOR=uug-ai
